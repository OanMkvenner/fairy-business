...
public class AdvancedOption {
    ...
    public Sprite sprite; // Sprite for option
    public TMP_FontAsset fontOverride; // optional override for Font of text

    /// <summary>
    /// Constructor
    /// </summary>
    /// <param name="nameText">Text for option</param>
    public AdvancedOption(string nameText, Sprite sprite = null, TMP_FontAsset fontOverride = null) {
        this.sprite = sprite;
...
public class AdvancedDropdown : MonoBehaviour{
    ...
    [SerializeField] Image targetImage; //Image component
    ...
    public UnityEvent<int> onChangedValue; //Action that executes when user select an option
    ...
    void Update() {
        ...
        switch (dropdownAnimationType) {
            case AnimationDropdownTypes.Shrinking:
                ratioShrinking = Mathf.Clamp(ratioShrinking + (speedOfShrinking * Time.unscaledDeltaTime) * curveShrinking.Evaluate(ratioShrinking), 0, 1);
            ...
            case AnimationDropdownTypes.Fading:
                ratioFading = Mathf.Clamp(ratioFading + (speedOfFading * Time.unscaledDeltaTime), 0, 1);
    ...
    public void AddOption(string targetOption, Sprite sprite = null, TMP_FontAsset fontOverride = null) {
        optionsList.Add(new AdvancedOption(targetOption, sprite, fontOverride));
    }
    ...
    public void OpenOptions() {
        ...
        for (int i = 0; i < optionsList.Count; i++) {
            ...
            newOption.InitOption(i, optionsList[i]);
    ...
    public void SelectOption(int id) {
        ...
        if (optionsList[id].fontOverride != null){
            targetText.font = optionsList[id].fontOverride;
        }
        if (targetImage != null){
            targetImage.sprite = optionsList[id].sprite;
        }
        for(int i = 0; i < spawnedList.Count; i++) {
            spawnedList[i].SetSelectState(i == value);
        }
        CloseOptions();
        onChangedValue.Invoke(id);
    }
}
