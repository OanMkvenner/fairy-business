...
#define ALGS_DEGREES_TO_RADS 0.017453292519943295
#define ALGS_ROUNDNUM(x) ((int)(x + 0.5f))
#define ALGS_BAD_EXTRA_RATIO_MARGIN 1.75
...
namespace algs {
    ...
    * @return
    */
    void ssc(std::vector<cv::KeyPoint> keyPoints, std::vector<cv::KeyPoint>& keypointsReturn, int numRetPoints,
        ...
        keypointsReturn.reserve(ResultVec.size());
		for (unsigned int i = 0; i < ResultVec.size(); i++)
            keypointsReturn.push_back(keyPoints[ResultVec[i]]);
	}
...
static inline void rectifyBoxes(const std::vector<BAD::BoxPairParams> &boxes_params,
  ...
  if (kp.angle == -1) {
    ...
    cosine = (kp.angle >= 0) ? float(cos(kp.angle * ALGS_DEGREES_TO_RADS)) : 1.f;
    sine = (kp.angle >= 0) ? float(sin(kp.angle * ALGS_DEGREES_TO_RADS)) : 0.f;
  ...
  for (size_t i = 0; i < boxes_params.size(); i++) {
    out_params[i].x1 = ALGS_ROUNDNUM(m00 * boxes_params[i].x1 + m01 * boxes_params[i].y1 + m02);
    out_params[i].y1 = ALGS_ROUNDNUM(m10 * boxes_params[i].x1 + m11 * boxes_params[i].y1 + m12);
    out_params[i].x2 = ALGS_ROUNDNUM(m00 * boxes_params[i].x2 + m01 * boxes_params[i].y2 + m02);
    out_params[i].y2 = ALGS_ROUNDNUM(m10 * boxes_params[i].x2 + m11 * boxes_params[i].y2 + m12);
    out_params[i].boxRadius = ALGS_ROUNDNUM(s * boxes_params[i].boxRadius);
  }
}
...
static inline bool isKeypointInTheBorder(const cv::KeyPoint &kp,
  ...
  cv::Size2f border(patch_size.width * s * ALGS_BAD_EXTRA_RATIO_MARGIN,
                    patch_size.height * s * ALGS_BAD_EXTRA_RATIO_MARGIN);
