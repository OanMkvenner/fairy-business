...
{
    [AddComponentMenu("UI/Toggleable Button Group", 31)]
    ...
    /// </remarks>
    public class ToggleableButtonGroup : UIBehaviour
    {
        ...
        protected List<ToggleableButton> m_Toggles = new List<ToggleableButton>();

        protected ToggleableButtonGroup()
        {}
        ...
        private void ValidateToggleIsInGroup(ToggleableButton toggle)
        {
            if (toggle == null || !m_Toggles.Contains(toggle))
                throw new ArgumentException(string.Format("ToggleableButton {0} is not part of ToggleableButtonGroup {1}", new object[] {toggle, this}));
        }
        ...
        public void NotifyToggleOn(ToggleableButton toggle, bool sendCallback = true)
        {
        ...
        public void UnregisterToggle(ToggleableButton toggle)
        {
        ...
        public void RegisterToggle(ToggleableButton toggle)
        {
            if (!m_Toggles.Contains(toggle))
        ...
        /// <summary>
        /// Ensure that the toggle group still has a valid state. This is only relevant when a ToggleableButtonGroup is Started
        ...
        public void EnsureValidState()
        {
            ...
            IEnumerable<ToggleableButton> activeToggles = ActiveToggles();

            if (activeToggles.Count() > 1)
            {
                ToggleableButton firstActive = GetFirstActiveToggle();

                foreach (ToggleableButton toggle in activeToggles)
                {
        ...
        /// </remarks>
        public IEnumerable<ToggleableButton> ActiveToggles()
        {
        ...
        /// </remarks>
        public ToggleableButton GetFirstActiveToggle()
        {
            IEnumerable<ToggleableButton> activeToggles = ActiveToggles();
